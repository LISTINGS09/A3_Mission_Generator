...
["DEBUG", format["Zone%1 - Creating Site: Count %2 at %3%4", _zoneID, _count, _centre, [""," [FORCED]"] select _forcePos]] call zmm_fnc_misc_logMsg;
...
["DEBUG", format["Zone%1 - Creating Site: %2 in %3 positions (%4 types)", _zoneID, _count, count _locations, count _buildingList]] call zmm_fnc_misc_logMsg;
...
for "_i" from 1 to _count do {	
	...
	if ( isMultiplayer && {allPlayers findIf { alive _x && _x distance2D _pos < 500 } > -1} ) exitWith { _centre = [] };
	...
	["DEBUG", format["Zone%1 - Area Site - Spawning: %5 of %6 - ID%2 (%3) at %4", _zoneID, _bID, _icon, _pos, _i, _count]] call zmm_fnc_misc_logMsg;
	...
	_tempGrp setGroupIdGlobal [format["ZMM_Z%1_G%2_SITE", _zoneID, _i]];
	...
	if (_showMarker) then {
		if (missionNamespace getVariable ["ZZM_Mode", 0] == 0) then {
			private _mrkr = createMarker [format["MKR_%1_SI_%2", _zoneID, _i], _pos getPos [25, random 360]];
			...
			_hdTrigger setTriggerStatements [  "this", format["'MKR_%1_SI_%2' setMarkerType '%3'; 'MKR_%1_SI_%2' setMarkerPos %4", _zoneID, _i, _icon, getPos _key], "" ];
		} else {
			private _mrkr = createMarkerLocal [format ["MKR_%1_SI_%2", _zoneID, _i], _pos];
