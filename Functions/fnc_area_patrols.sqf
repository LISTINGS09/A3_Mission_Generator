...
_fnc_spawnGroup = {
	...
	_vSide = missionNamespace getVariable [format["ZMM_%1_enemySide", _zoneID], EAST];
	...
	for [{_i = 0}, {_i < _count}, {_i = _i + 1}] do {
		...
		if (typeName _type == typeName "") then {
			if (isClass (configFile >> "CfgVehicles" >> _type)) then {
				...
				if (side driver _veh != _vSide) then {
					_sideGrp = createGroup [_vSide, TRUE];
			...
			_group = [[0,0,0], _vSide, _type] call BIS_fnc_spawnGroup;
