...
["DEBUG", format["Zone%1 - Creating Static: Count %2 at %3%4", _zoneID, _count, _centre, [""," [FORCED]"] select _forcePos]] call zmm_fnc_misc_logMsg;
...
private _vehAAA = missionNamespace getVariable [format["ZMM_%1_AAA",_side],_vehArr];
private _vehART = missionNamespace getVariable [format["ZMM_%1_ART",_side],_vehArr];
...
["DEBUG", format["Zone%1 - Creating Static: %2 in %3 positions (%4 types)", _zoneID, _count, count _locations, count _buildingList]] call zmm_fnc_misc_logMsg;
...
for "_i" from 1 to _count do {	
	...
	if ( isMultiplayer && {allPlayers findIf { alive _x && _x distance2D _pos < 500 } > -1} ) exitWith { _centre = [0,0,0] };
	...
	["DEBUG", format["Zone%1 - Area Static - Spawning: %5 of %6 - ID%2 (%3) at %4", _zoneID, _bID, _icon, _pos, _i, _count]] call zmm_fnc_misc_logMsg;
	...
	_tempGrp setGroupIdGlobal [format["ZMM_Z%1_G%2_STATIC", _zoneID, _i]];
