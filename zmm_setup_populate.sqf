// Populates a zoneID with a task (if not ambient) and units - Adds patrols (UPS Script), garrison (Spawns) and QRF (QRF Script)
...
// Set-up QRF
if (count (missionNamespace getVariable [ format["ZMM_%1_QRFLocations", _zoneID], [] ]) > 0 && {_qrfTime > 0}) then {
