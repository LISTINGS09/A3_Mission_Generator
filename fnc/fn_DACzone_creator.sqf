...

{
	...
	_zoneList_areas_all pushBack _zoneName;	// Add the Zone to the masterlist.
...
// UPDATE We're going to assume all zones start inactive.
...
// Future version of Arma can activate a trigger from any playable unit.
//_zoneTrigger setTriggerActivation ["ANYPLAYER", "PRESENT", true];

//[format["[TG] DEBUG (DACZoneCreator): Trigger [%1, %2, %3]", _missionName, _zoneList_areas_all, (_zoneList_areas_all joinString ","), _zoneList_pos]] call tg_fnc_debugMsg;
...
if (count _zoneList_areas_active > 0) then {
	...
	_activate_code = format[
		"if !(missionNamespace getVariable ['%1',false]) then { 
			[] spawn {
				waitUntil{ sleep 1; if (DAC_NewZone != 0) then { ['[TG] DEBUG: (DAC) Activate waiting for DAC_NewZone (%1)'] call tg_fnc_debugMsg; }; DAC_NewZone == 0; };
...
private _deactivate_code = 
	format["if (missionNamespace getVariable ['%1',false]) then { 
			[] spawn { 
				waitUntil{ sleep 1; if (DAC_NewZone != 0) then { ['[TG] DEBUG: (DAC) Delete waiting for DAC_NewZone (%1)'] call tg_fnc_debugMsg; }; DAC_NewZone == 0; };
...
if (count _zoneList_areas_active > 0) then {
	_deactivate_code = _deactivate_code + format[
		" else {
			[] spawn {
				waitUntil{ sleep 1; if (DAC_NewZone != 0) then { ['[TG] DEBUG: (DAC) Deactivate waiting for DAC_NewZone (%1)'] call tg_fnc_debugMsg; }; DAC_NewZone == 0; };
