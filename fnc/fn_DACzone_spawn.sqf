...
waitUntil{sleep 3; if (DAC_NewZone != 0) then {[format["[TG] DEBUG: %1 waiting for DAC_NewZone (%2)", _zoneName, DAC_NewZone]] call tg_fnc_debugMsg;}; DAC_NewZone == 0; };

// Create the DAC Zone
[format["[TG] DEBUG: (DAC_fNewZone) [%1, %2, %2, 0, 0, %3]", _zonePos, _size, [_zoneName] + _DACinit]] call tg_fnc_debugMsg;
...
// Looks like error was related to DAC_Init_Camps not resetting.
waitUntil{	sleep 5;
			_waitTime = _waitTime + 5; 
			if (DAC_NewZone != 0 && _waitTime > 60 && _waitTime < 100) then {
			...
			if (DAC_NewZone != 0 && _waitTime > 120) then {
				[format["[TG] ERROR: %1 Resetting DAC_NewZone was %2, DAC_Init_Camps was %3", _zoneName, DAC_NewZone, DAC_Init_Camps]] call tg_fnc_debugMsg; 
				format["ERROR: DAC_NewZone (%1) - RTB/Endex after mission!", _zoneName] remoteExec ["SystemChat"];
				DAC_NewZone = 0; 
			};
			DAC_NewZone == 0; 
		};
