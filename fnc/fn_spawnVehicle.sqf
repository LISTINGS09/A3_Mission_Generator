...
// _count		INT			How many vehicles to create (won't ever be more than _foundObjs)
...
params ["_searchPos", "_inDist", "_count", ["_spawnType", [], [[]]]];

_spawnType params [["_spawnAir", true, [true]],["_spawnLand", true, [true]],["_spawnSea", true, [true]],["_milSide", "Civ", [""]]];
...
switch (toLower _milSide) do {
	case "west": {
		_milSide = toLower _milSide;
...
//_foundObjs = nearestObjects [_searchPos, ["All"], _inDist];
_foundObjs = _searchPos nearObjects ["Building", _inDist];

if (count _foundObjs == 0) exitWith {};

// Damage vehicle function.
private _fnc_damageVehicle = {
	params ["_veh"];
	_veh setFuel random 0.25;
	_veh setDamage random 0.55;
...
[format ["[TG] DEBUG spawnVehicle: Starting - 0/%1 with %2 objects found in R%7 [%3,%4,%5,%6]", _count, count _foundObjs, _spawnAir, _spawnLand, _spawnSea, _milSide, _inDist]] call tg_fnc_debugMsg;
	...
	if _spawnLand then {
		switch (typeOf _building) do {
			...
			case "Land_House_Big_03_F": {
				[
					_forEachIndex,
					selectRandom _vL_sml,
					_building modelToWorld [17,-3,0.2],
					(getDir _building) + selectRandom [90,270]
				] call _fnc_createVehicle;	
			};
			// Garage House (Apex)
			case "Land_GarageShelter_01_F": {
				[
					_forEachIndex,
					selectRandom _vL_sml,
					_building modelToWorld [2.5,0,0.3],
					(getDir _building) + selectRandom [0,180]
				] call _fnc_createVehicle;	
			}
			// Warehouse (Apex)
			case "Land_Addon_05_F" {
				[
					_forEachIndex,
					selectRandom _vL_sml,
					_building modelToWorld [-1,-14,0.3],
					(getDir _building) + selectRandom [0,180]
				] call _fnc_createVehicle;	
			};
			// Garage + Warehouse Roof (Apex)
			case "Land_i_Garage_V2_F"; case "Land_i_Garage_V1_F"; case "Land_WarehouseShelter_01_F": {
				[
					_forEachIndex,
					selectRandom _vL_sml,
					_building modelToWorld [0,0,0.2],
					(getDir _building) + selectRandom [0,180]
				] call _fnc_createVehicle;	
			}
			// Yard Roof
			...
			case "Land_SM_01_shed_unfinished_F"; case "Land_SM_01_shed_F"; case "Land_u_Shed_Ind_F"; case "Land_i_Shed_Ind_F": {
				[
					_forEachIndex,
					selectRandom (_vL_lrg + _vL_sml),
					_building modelToWorld [9,-5,0.2],
					(getDir _building) + selectRandom [270,90]
				] call _fnc_createVehicle;		
			};
			...
			case "Land_fs_roof_F"; case "Land_FuelStation_Shed_F"; case "Land_FuelStation_01_roof_F"; case "Land_FuelStation_02_roof_F": {
				...
				[
					_forEachIndex,
					selectRandom (_vU + _vL_lrg + _vL_sml),
					_rPos,
					(getDir _building) + selectRandom [270,90]
				] call _fnc_createVehicle;
			};
		...
		switch (typeOf _building) do {
			case "Land_Airport_01_hangar_F"; case "Land_TentHangar_V1_F": {
					[
						_forEachIndex,
						selectRandom _vA,
						_building modelToWorld [0,-25,0.2],
						(getDir _building) + selectRandom [0,180]
					] call _fnc_createVehicle;		
			};
			case "Land_Hangar_F": {
					[
						_forEachIndex,
						selectRandom _vA,
						_building modelToWorld [0,-35,0.2],
						(getDir _building) + selectRandom [0,180]
					] call _fnc_createVehicle;		
			};
		};
