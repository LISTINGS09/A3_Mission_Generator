...
for "_i" from 1 to (_civMax / 2) do {
	...
	_bombTrigger setTriggerStatements [  "this", 
									...
									[ZMM_%1_IED_%2] spawn {
										params ['_unit'];
										waitUntil { sleep 1; playSound3D ['A3\sounds_f\sfx\beep_target.wss', _unit, false, getPosASL _unit, 1, 0.5, 50]; (!alive _unit || allPlayers findIf { alive _x && _unit distance _x < 5 } >= 0) };
