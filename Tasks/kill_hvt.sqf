...
private _alias = ["Butcher", "Diablo", "Angel", "Scorpion", "Hyena", "Jackal", "Prophet"];

private _headText = if (isClass (configFile >> "CfgWeapons" >> headgear (leader _milGroup))) then {
		format["<br/><br/>The target is known to be wearing a <font color='#00FFFF'>%1</font>.<br/><br/><img image='%2' width='60'/>", getText (configFile >> "CfgWeapons" >> headgear (leader _milGroup) >> "displayName"), getText (configFile >> "CfgWeapons" >> headgear (leader _milGroup) >> "picture")];
	} else { "" };
...
_objTrigger setTriggerStatements [ 	format["!alive ZMM_%1_HVT", _zoneID], 
									...
									"" ];
// Create Task
_missionTask = [format["ZMM_%1_TSK", _zoneID], TRUE, [format["<font color='#00FF80'>Mission (#ID%1)</font><br/>", _zoneID] + format[selectRandom _missionDesc + _headText, _locName, name leader _milGroup, selectRandom _alias], ["Killer"] call zmm_fnc_nameGen, format["MKR_%1_LOC", _zoneID]], _centre, "AUTOASSIGNED", 1, FALSE, TRUE, "target"] call BIS_fnc_setTask;
