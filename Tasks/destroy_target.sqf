...
private _tempArr = (_tskCentre nearObjects ["Building", 200]) select { typeOf _x in _targetTypes };
...
if (count _tempArr == 0) then { 
	...
	private _targetObj = createVehicle [selectRandom _targetTypes, _tskPos, [], 0, "NONE"];
	_targetObj setVectorUp [0,0,1];
	_tskPos = getPos _targetObj;
...
// Create Task
_missionTask = [format["ZMM_%1_TSK", _zoneID], true, [format["<font color='#00FF80'>Mission (#ID%1)</font><br/>", _zoneID] + format[selectRandom _tskDesc, _locName, count _tskActivation, if (count _tskActivation > 1) then { " Buildings" } else { " Building" }], ["Demolition"] call zmm_fnc_nameGen, format["MKR_Z%1_LOC", _zoneID]], _tskCentre, "CREATED", 1, false, true, "destroy"] call BIS_fnc_setTask;
