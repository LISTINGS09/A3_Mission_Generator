...
_missionDesc = [
		"Secure the area around <font color='#0080FF'>%1</font color> then locate and mark the locations of <font color='#00FFFF'>%2 Container(s) (%3)</font color> in the area.",
		"Investigate <font color='#0080FF'>%1</font color> by searching the surrounding area and locating <font color='#00FFFF'>%2 Container(s) (%3)</font color> somewhere in the area.",
		"<font color='#0080FF'>%1</font color> is occupied by enemy forces, eliminate them and secure the area by claiming the <font color='#00FFFF'>%2 Container(s) (%3)</font color>.",
		"Hunt down the locations of <font color='#00FFFF'>%2 Container(s) (%3)</font color> somewhere in <font color='#0080FF'>%1</font color>.",
		"Enemy forces have occupied <font color='#0080FF'>%1</font color>, eliminate them and find the <font color='#00FFFF'>%2 Container(s) (%3)</font color> in the area.",
		"Locate the <font color='#00FFFF'>%2 Container(s) (%3)</font color> hidden somewhere in <font color='#0080FF'>%1</font color>."
	];
...
{	
	...
	// Add players action
	[_item, 
		"<t color='#00FF80'>Mark Location</t>", 
		...
		{ 	
			...
			format[ "MKR_%1_ITEM%2", _zoneID, _itemNo ] setMarkerColor format["Color%1",side player];
			format[ "MKR_%1_ICON%2", _zoneID, _itemNo ] setMarkerColor format["Color%1",side player];
	...
	_itemActivation pushBack format["(missionNamespace getVariable [ 'ZMM_%1_ITEM%2', FALSE ])", _zoneID, _forEachIndex];
	
	// Get item description for task.
	_itemText pushBack ([getText (configFile >> "CfgVehicles" >> _itemType >> "displayName"),"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_- "] call BIS_fnc_filterString);
	
	_itemDist = if (_isSmall) then { 75 } else { 50 };
	
	_relPos = [ position _item, random _itemDist, random 360 ] call BIS_fnc_relPos;	
	_mrk = createMarker [ format[ "MKR_%1_ITEM%2", _zoneID, _forEachIndex ], _relPos ];
...
// Create Task
_missionTask = [format["ZMM_%1_TSK", _zoneID], TRUE, [format["<font color='#00FF80'>Mission (#ID%1)</font><br/>", _zoneID] + format[selectRandom _missionDesc, _locName, count _itemActivation, (_itemText joinString ", ")], ["Search"] call zmm_fnc_nameGen, format["MKR_%1_LOC", _zoneID]], _centre, "CREATED", 1, FALSE, TRUE, "attack"] call BIS_fnc_setTask;
