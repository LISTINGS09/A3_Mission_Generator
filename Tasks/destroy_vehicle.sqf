...
private _vehTank = missionNamespace getVariable[format["ZMM_%1_Heavy",_enemySide],[""]];
...
for "_i" from 1 to (missionNamespace getVariable ["ZZM_ObjectiveCount", 1]) do {
	...
	["DEBUG", format["destroy_vehicle (%1) - Creating %2 at %3", _zoneID, _vehClass, _vehPos]] call zmm_fnc_misc_logMsg;
	...
	if (alive _vehObj) then {
		...
		if (isOnRoad _vehObj) then { 
			...
			_nul = [driver _vehObj, format["MKR_Z%1_MIN", _zoneID], "SHOWMARKER"] spawn zmm_fnc_aiUPS;
		} else {
			_vehObj setDir random 360;
			if (random 100 < 25) then { _nul = [driver _vehObj, format["MKR_Z%1_MIN", _zoneID], "SHOWMARKER"] spawn zmm_fnc_aiUPS };
		};
		...
		_tskActivation pushBack format["(missionNamespace getVariable ['ZMM_%1_OBJ_%2_DEAD', false])", _zoneID, _vehNo];
...
// Create Task
_missionTask = [format["ZMM_%1_TSK", _zoneID], true, [format["<font color='#00FF80'>Mission (#ID%1)</font><br/>", _zoneID] + format[_tskDesc, count _tskActivation, if (count _tskActivation > 1) then {"Vehicles"} else {"Vehicle"}, _locName], ["Destroy"] call zmm_fnc_nameGen, format["MKR_Z%1_LOC", _zoneID]], _tskCentre, "CREATED", 1, false, true, "destroy"] call BIS_fnc_setTask;
