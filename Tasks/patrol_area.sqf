...

{
	...
	_gridRef = format["%1%2", _gridRef#0, _gridRef#1];
	...
	if (!(surfaceIsWater _x) && !(_gridRef in _areaGrids)) then {
		...
		private _mrk = createMarker [ format[ "MKR_%1_ICON_%2", _zoneID, _forEachIndex ], _gridPos];
		...
		private _childTask = [[format["ZMM_%1_SUB_%2", _zoneID, _forEachIndex], format['ZMM_%1_TSK', _zoneID]], true, [format["Patrol within the area of %1.", _gridRef], format["Position #%1", _forEachIndex + 1], format["MKR_%1_ICON_%2", _zoneID, _forEachIndex]], _gridPos, "CREATED", 1, false, true, format["move%1", _forEachIndex + 1]] call BIS_fnc_setTask;
		...
		_childTrigger setTriggerTimeout [9, 9, 9, false];
		_childTrigger setTriggerStatements [ "this",
			format["missionNamespace setVariable ['ZMM_%1_OBJ_%2', true, true]; ['ZMM_%1_SUB_%2', 'Succeeded', true] spawn BIS_fnc_taskSetState; deleteMarker 'MKR_%1_ICON_%2';", _zoneID, _forEachIndex],
			"" ];
			
		_areaActivation pushBack format["(missionNamespace getVariable ['ZMM_%1_OBJ_%2', false])", _zoneID, _forEachIndex];
