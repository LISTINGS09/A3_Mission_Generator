...

{
	...
	
	{
		...
		if !([_locPos] call tg_fnc_inSafeZone) then {
			...
			if tg_spawnAmbientObjects then {
				...
				_vehTrigger setTriggerArea [round (_activateRange / 2), round (_activateRange / 2.5), 0, true, 400];
				...
				// The below spawns all vehicles on the map at start - SMALL MAPS ONLY - Group Limits!
				// [_locPos, _locRadius, _locVehicles * 4, _enemySide, _enemySoldier] spawn tg_fnc_spawnMilitary;		
				// [_locPos, _locRadius, _locVehicles + random (_locVehicles / 2), [_spawnA, _spawnL, _spawnS, if _spawnM then {_enemySide} else {civilian}]] spawn tg_fnc_spawnVehicle;
			};
		};
